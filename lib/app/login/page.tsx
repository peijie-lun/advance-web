...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    const { error } = await supabase.auth.signInWithPassword({ email, password });// 使用 signInWithPassword 方法進行登入
...
//檢查 email 和 password 是否有填
// 用 supabase.auth.signInWithPassword 登入
// 登入失敗 alert 錯誤訊息
// 登入成功後 0.5 秒導向 /order/orderlist
  return (
    <Box
      sx={{
        minHeight: '100vh',
