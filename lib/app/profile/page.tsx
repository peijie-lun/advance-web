...
//   //useAuth() 取得目前登入使用者資料與角色

// profile 儲存個人資料（從 Supabase 拿到）

// loading 控制資料是否還在載入

// isSaving 控制儲存中狀態，避免多次點擊
  ...
  const handleSnackbarClose = (event: React.SyntheticEvent | Event, reason?: string) => {
    if (reason === 'clickaway') {// 點擊外部不關閉
      return;
    }
    setSnackbarOpen(false);// 關閉提示
  };
...
// //登入後，useEffect 監聽 user 狀態，呼叫 fetchProfile()：
//  - 用 user.id 去 Supabase profiles 資料表查詢個人資料。
//  - 如果查不到，建立一筆預設資料。
//  - 查到資料後，存到 profile 狀態。
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      let { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('id', user.id)//id = user.id 查詢
        .single();// single() 確保只取一筆資料

      if (error && error.code === 'PGRST116') {
        // 資料庫無資料，創建預設資料
        const newProfileData = {// 準備預設資料
          id: user.id,
          email: userEmail,
          username: '',
          full_name: '',
        ...
        const { data: insertData, error: insertError } = await supabase.from('profiles').insert([newProfileData]).select().single();// 插入新資料
//.select():插入後順便回傳這筆資料的完整欄位
        if (insertError) {
        ...
        // 成功獲取資料
        setProfile(data);
//如果從資料庫查詢 profiles 成功拿到資料（data 有值），就把這筆資料存進 profile 狀態（setProfile(data)），讓畫面顯示出來。這是用來顯示使用者的個人資料。
      } else if (error) {
...
// useEffect 監聽 user 狀態變化，當 user 有值時觸發 fetchProfile 函式
// fetchProfile 會去 Supabase 的 profiles 資料表查詢目前使用者的個人資料
// 如果沒有資料，會自動建立一筆預設資料
// 查到資料後，存到 profile 狀態，供畫面渲染使用
  ...
  async function handleSave() {
    ...
    if (!profile.username || profile.username.trim() === '') {//trim() 去除前後空白 // 如果用戶名稱是空的 //只包含空白
    ...
    const { error } = await supabase
      ...
      .eq('id', profile.id);// 根據 id 更新對應的資料列 //更新 profiles 表中 id 等於 profile.id 的那一列資料，將使用者在表單中修改的值更新回資料庫。//eq="等於"的意思
...
//ProtectedProfilePage 是一個元件，回傳一個被 ProtectedRoute 包住的 ProfilePage。
// ProtectedRoute 會檢查使用者是否有登入，沒登入就導向登入頁，有登入才顯示 ProfilePage。
// export default ProtectedProfilePage 讓這個元件成為 profile 頁面的預設匯出。
//這樣任何人要看 profile 頁面前都會先經過 ProtectedRoute 的檢查